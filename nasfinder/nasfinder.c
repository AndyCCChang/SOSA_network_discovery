...
int FillBuffer(char *buf, int opcode, char *netinfo_multi)
{
	...
	*/
	...
	if(opcode == 1 && netinfo_multi) {
		...
		//Minging.Tsai. 2013/10/25.  Add Helios serial to go with nasfinder.
		/*
		FILE *fp;
		char Helios_sn[19];
		fp = fopen("/islavista/config/NAS_APP/helios_sn", "r");
		fgets(Helios_sn, 19, fp);
		//printf("Helios_sn=\"%s\"\n", Helios_sn);
		fclose(fp);
		memcpy(buf+112, Helios_sn, 19);
		*/
...
#define MAX_IFS 20
	...
	for (i=0; i < 2; i++)//Broadcast 2 requests to make sure we can get every NAS gateway.
	{
	...
	if(opcode < 1)
	{
		...
		for (i=0; i < 1000; i++)//1000
		{
			...
			if(count > 0) {
				//printf("count=%d, peeraddr=%s\n", count, str);
			}
			//printf("size of br.bootfile=%d\n", sizeof(br.bootfile));
			//printf("size of br=%d\n", sizeof(br));
			//printf("br.bootfile=%s\n", br.bootfile);
			if (count > 0) {
				...
				else
				{
					...
					//printf("br.servername+strlen(MSGKEY)=%s\n", br.servername+strlen(MSGKEY));
				...
				zero_counter = 0;//Set zero if receive something.
			}
			else //count <= 0
				zero_counter++;//increase when nothing receive

			//printf("zero_counter=%d\n", zero_counter);
			if(zero_counter >= 200) {
				//printf("break when zero_counter >= 100\n");
				break;
			}
			//usleep(1000 * waitTime);//1000
			//Minging.Tsai. 2013/8/8
			usleep(100);// actually wait 10000us in Helios, so in order to calibrate this situation, 
			//I change it to usleep(100).
